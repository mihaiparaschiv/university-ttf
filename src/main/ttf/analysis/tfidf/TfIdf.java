...
public class TfIdf {
	...
	public void SummaryAnalyzer() {
		...
		try {
			stopwords = filterComments(StringUtils.split(FileUtils
					.readFileToString(new File("resources/stopwords.txt"),
							"UTF-8")));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public Collection<Token> GetUrlTokens(String uri) throws Exception {
		...
		StringExtractor se = new StringExtractor(uri); // uses HTML parser -
		// StringExtractor
