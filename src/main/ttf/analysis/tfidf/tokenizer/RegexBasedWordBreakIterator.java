...
/**
 * A word break iterator that allows its default behavior for the Locale to be
 * overriden by supplied regular expression rules.
 */
public class RegexBasedWordBreakIterator extends BreakIterator {
	...
	/**
	 * We are only interested in overriding next(), since that is the only
	 * method our WordTokenizer depends on. A more complete implementation
	 * should try to extend all the methods below. In our case, all we do is
	 * delegate to the underlying BreakIterator.wordInstance for the other
	 * methods.
	 * 
	...
	public int next() {
		...
		for (;;) {
			...
			if (wordBreakIterator.isBoundary(next)) {
				...
				if (currentChar == '<') {
					// when '<' encountered, return the token so far, then set
					// the flag
					inXml = true;
					break;
				}
				if (currentChar == '-') {
					// when a '-' is encountered, we need to keep going until we
					// encounter
					// a whitespace boundary. But we don't return the token in
					// this case.
					inHyphenatedToken = true;
				}
				if (inXml) {
					// xml pattern tokenization, keep accumulating until the
					// closing
					// tag is encountered, then advance the wordBreakIterator to
					// the
					...
					if (Character.isLetterOrDigit(currentChar)) {
						// if the next boundary starts with a letter or digit,
						// there is
						// not much confusion as to whether it is a boundary or
						// not.
						break;
					}
					char precedingChar = text.substring(next - 1, next).charAt(
							0);
					char followingChar = text.substring(next + 1, next + 2)
							.charAt(0);
					System.out.println("text[" + next + "]=" + currentChar
							+ ", preceding char=" + precedingChar
							+ ", following char=" + followingChar);
					// Default rule: If the boundary is not a letter or digit,
					// it can be one of
					// the many punctuation chars. If the character before and
					// after the boundary
					// character is alphanumeric, then we consider this a
					// boundary.
