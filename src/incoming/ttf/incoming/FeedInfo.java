...
public class FeedInfo implements Comparable<FeedInfo> {
	...
	public int compareTo(FeedInfo o) {
		return Long.signum(lastCheck.getTime() + interval * MINUTE
				- o.lastCheck.getTime() - o.interval * MINUTE);
	}
	...
	public void update(SyndFeed feed) {
		...
		if (entries.size() > 1) {
			...
			int entriesAverage = (int) (Math.abs(firstEntry.getPublishedDate()
					.getTime()
					- lastEntry.getPublishedDate().getTime()) / (MINUTE * (entries
					.size() - 1)));
			if (interval == 0)
				interval = entriesAverage;
			else
				interval = (interval * (MAGIC_NUMBER - 1) + entriesAverage)
						/ MAGIC_NUMBER;
		}
	}
}