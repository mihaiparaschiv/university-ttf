...
public class FeedReader {
	...
	public void InitializeQueue() throws SQLException, MalformedURLException {
		...
		String sql = "SELECT address, retrievalInterval, lastChecked FROM "
				+ SOURCES;
	...
	public void run() throws IllegalArgumentException, FeedException,
		...
		while (queue.size() > 0) {
			...
			Timestamp nextCheck = new Timestamp(feedInfo.lastCheck.getTime()
					+ feedInfo.interval * FeedInfo.MINUTE);
	...
	private void process(FeedInfo feedInfo) throws IllegalArgumentException,
			FeedException, IOException, SQLException {
		QueryRunner run = new QueryRunner(dataSource);
		FeedEntryParser entryParser = new FeedEntryParser();

		String check = "SELECT * FROM " + INCOMINGARTICLES
				+ " WHERE address = ?";
		...
		for (Object e : feed.getEntries()) {
			...
			// Add if new
			List l = run.query(check, new ArrayListHandler(), article
					.getAddress());
			if (l.size() == 0) {
				run.update(sql, //
						article.getAddress(), //
		...
		sql = "UPDATE "
				+ SOURCES
				+ " SET lastChecked = ?, retrievalInterval = ? where address = ?";
		run
				.update(sql, feedInfo.lastCheck, feedInfo.interval,
						feedInfo.address);
	}

	public static void main(String[] args) throws ConfigurationException,
			SQLException, IllegalArgumentException, FeedException, IOException {
